...
export default async function HomePage() {
  ...
  const draftCertificates = certificates.filter(cert => cert.certificate.status === 'draft').length;
  const completedCertificates = certificates.filter(cert => cert.certificate.status === 'completed').length;
  const issuedCertificates = certificates.filter(cert => cert.certificate.status === 'issued').length;
          ...
          <CardContent>
            {recentCertificates.length > 0 ? (
              <div className="space-y-3">
                {recentCertificates.map((cert) => (
                  <div key={cert.certificate.id} className="flex items-center justify-between">
                    <div>
                      <div className="font-medium">{cert.certificate.certificateNumber}</div>
                      <div className="text-sm text-muted-foreground">
                        {cert.customer?.name} • {cert.certificate.certificateType}
                      </div>
                    </div>
                    <div className={`px-2 py-1 rounded-full text-xs ${
                      cert.certificate.status === 'issued' ? 'bg-green-100 text-green-800' :
                      cert.certificate.status === 'completed' ? 'bg-blue-100 text-blue-800' :
              ...
              <div className="text-center py-6">
                ...
                <p className="mt-1 text-sm text-gray-500">Get started by creating your first certificate.</p>
                <div className="mt-6">
                  <Button asChild>
                    <Link href="/certificates/new">
                      <FileText className="mr-2 h-4 w-4" />
